 ...
 */
export function createStatsCard(label: string, value: string | number, icon?: string): HTMLElement {
 ...
 * Progress Bar
 */
export function createProgressBar(current: number, total: number, label?: string): HTMLElement {
  const container = document.createElement('div');
 ...
 */
export function createQuestionStateIndicator(
  state: 'not-visited' | 'visited' | 'answered' | 'marked'
): HTMLElement {
...
export class Modal {
  ...
  getContent(): HTMLElement {
    return this.modal.querySelector('div:last-child') as HTMLElement;
  }
}
 ...
 * Empty State
 */
export function createEmptyState(icon: string, title: string, description: string): HTMLElement {
  const container = document.createElement('div');
  container.style.cssText = `
    text-align: center;
    padding: 3rem 1rem;
