...
export async function createTestsPage(): Promise<HTMLElement> {
  ...
  try {
    ...
    if (configs.length === 0) {
      main.appendChild(
        createEmptyState('📝', 'No Tests', 'Create a test configuration to get started.')
      );
    } else {
        ...
        .forEach((config) => {
          ...
          startBtn.addEventListener('click', async (e) => {
            ...
            try {
              ...
              const session: TestSession = { ...sessionData, studentId: currentStudent.id };
    ...
    main.innerHTML = '';
    main.appendChild(
      createEmptyState('❌', 'Error', 'Failed to load tests. Please refresh the page.')
    );
  }
