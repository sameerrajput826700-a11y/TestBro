...
export async function createNewTestPage() {
    ...
    try {
        ...
        if (questions.length === 0) {
            main.appendChild(createEmptyState('📚', 'No Questions Available', 'Import questions first before creating a test.'));
            ...
            // Test Description
            const descGroup = createFormGroup('Test Description', 'textarea', 'testDescription', 'Test your knowledge');
            ...
            const subjects = new Set(questions.map((q) => q.subject).filter((s) => Boolean(s)));
            const topics = new Set(questions.map((q) => q.topic).filter((t) => Boolean(t)));
            const difficulties = new Set(questions
                .map((q) => q.difficulty)
                .filter((d) => d !== undefined));
            const createCheckboxGroup = (label, items, name) => {
            ...
            if (subjects.size > 0)
                filterDiv.appendChild(createCheckboxGroup('Subjects', subjects, 'subjects'));
            if (topics.size > 0)
                filterDiv.appendChild(createCheckboxGroup('Topics', topics, 'topics'));
            if (difficulties.size > 0)
                filterDiv.appendChild(createCheckboxGroup('Difficulty', difficulties, 'difficulties'));
            ...
            // Number of Questions
            const numQGroup = createFormGroup('Number of Questions', 'number', 'numberOfQuestions', Math.min(100, questions.length).toString(), true, {
                min: '1',
            ...
            const correctGroup = createFormGroup('Marks for Correct Answer', 'number', 'marksPerCorrect', '4', true, { step: '0.25' });
            marksSection.appendChild(correctGroup);
            const negativeGroup = createFormGroup('Negative Marks for Wrong Answer', 'number', 'negativeMarking', '1', true, { step: '0.25' });
            ...
            form.addEventListener('submit', async (e) => {
                ...
                const action = e.submitter?.value || 'start';
                const config = {
                    id: generateId('test'),
                    ...
                    marksPerCorrect: parseFloat(formData.get('marksPerCorrect')),
                    negativeMarking: parseFloat(formData.get('negativeMarking')),
                    passPercentage: parseFloat(formData.get('passPercentage')),
        ...
        main.innerHTML = '';
        main.appendChild(createEmptyState('❌', 'Error', 'Failed to create test. Please refresh the page.'));
    }
    container.appendChild(main);
    return container;
}
function createFormGroup(label, type, name, defaultValue = '', required = false, attributes = {}) {
    const group = document.createElement('div');
