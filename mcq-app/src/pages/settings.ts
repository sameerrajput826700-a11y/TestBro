...
import type { AppSettings, BackupData } from '../models/types';
...
export async function createSettingsPage(): Promise<HTMLElement> {
  ...
  try {
    ...
    const settings: AppSettings = savedSettings || defaultSettings;
    ...
    const themeCard = createSettingsCard('Theme', () => {
      ...
      themes.forEach((theme) => {
        ...
        input.addEventListener('change', () => {
          settings.theme = theme.value as any;
    ...
    const defaultsCard = createSettingsCard('Test Defaults', () => {
      ...
      const createInput = (label: string, key: keyof AppSettings, type: string = 'number') => {
        ...
        input.addEventListener('change', () => {
          if (type === 'number') {
            (settings as any)[key] = parseFloat(input.value);
          } else {
    ...
    const optionsCard = createSettingsCard('Test Options', () => {
      ...
      const createCheckbox = (label: string, key: keyof AppSettings) => {
    ...
    main.appendChild(
      createEmptyState('❌', 'Error', 'Failed to load settings. Please refresh the page.')
    );
  }
...
function createSettingsCard(title: string, contentFn: () => HTMLElement): HTMLElement {
