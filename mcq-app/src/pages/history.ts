...
export async function createHistoryPage(): Promise<HTMLElement> {
  ...
  if (!currentStudent) {
    main.appendChild(
      createEmptyState('🔒', 'Login Required', 'Please log in to view your test history.')
    );
  ...
  try {
    ...
    if (results.length === 0) {
      main.appendChild(
        createEmptyState('📋', 'No Test History', 'You haven\'t taken any tests yet.')
      );
    ...
    main.innerHTML = '';
    main.appendChild(
      createEmptyState('❌', 'Error', 'Failed to load history. Please refresh the page.')
    );
  }
