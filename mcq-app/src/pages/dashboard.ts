...
export async function createDashboardPage(): Promise<HTMLElement> {
  ...
  if (!currentStudent) {
    main.appendChild(
      createEmptyState('🔒', 'Login Required', 'Please log in to access the student dashboard.')
    );
  ...
  try {
    ...
    statsGrid.appendChild(
      createStatsCard('Total Questions', questions.length, '📚')
    );

    const totalTests = studentResults.length;
    statsGrid.appendChild(
      createStatsCard('Tests Taken', totalTests, '✔️')
    );
    ...
    const averageScore = totalTests > 0
      ...
      : '0.0';
    statsGrid.appendChild(
      createStatsCard('Average Score', `${averageScore}%`, '📊')
    );
    ...
    const bestScore = totalTests > 0
      ...
      : '0.0';
    statsGrid.appendChild(
      createStatsCard('Best Score', `${bestScore}%`, '🏆')
    );
    ...
    if (questions.length === 0) {
      ...
      emptyContainer.appendChild(
        createEmptyState('📚', 'No Questions Yet', 'Import a question bank to get started.')
      );
    ...
    main.innerHTML = '';
    main.appendChild(
      createEmptyState('❌', 'Error', 'Failed to load dashboard. Please refresh the page.')
    );
  }
